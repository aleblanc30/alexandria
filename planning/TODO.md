...
- [ ] **`upsert_chunks` doesn't batch under Chroma's max batch size** — `pka/storage/vector_store.py:202` passes the whole chunk list straight to `collection.upsert()`; a large document (seen: 14584 chunks from a Firefox fetch) blows past Chroma's `max_batch_size` (5461) and raises `chromadb.errors.InternalError`, so the whole doc's embedding fails (`embed_fetched_text` in `runners/firefox.py:109`) and it's left with zero chunks. Chunk the `ids`/`texts`/`metadatas` lists into `get_collection()._client.get_max_batch_size()`-sized (or a fixed safe constant) slices before upserting.
...
- [ ] **Source probes redundantly reload the same connector data** — `count_pending_metadata` and `source_corpus_size` (`pka/ingestion/pending_metadata.py`) each independently call `load_bookmarks()` / `try_load_calibre_books()` / `try_scan_images()` and are cached separately (keyed by `("pending", src)` vs `("corpus", src)`); on every sync start the cache is invalidated (`invalidate_source_probes`) and the actual sync job (`firefox_sync.py` etc.) also calls the connector directly, so one ingest run re-parses the full Firefox `places.sqlite` (20k+ rows), Calibre library, and image folder 2-3x each ("Found N / Loaded N" logged repeatedly back-to-back). Share one computed result across the pending/corpus probes and the real sync fetch within a job instead of three independent reads.

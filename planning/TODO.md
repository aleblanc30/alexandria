...
- [ ] **`adaptive_cluster_params` manufactures the clustering noise** — HDBSCAN infers its own cluster count, but `pka/clustering/engine.py:289` back-solves `min_cluster_size` from a `target_clusters` that `min(12, …)` pins at 12 for any corpus over 144 docs. So `min_cluster_size` scales with the archive instead of staying fixed: **744** and `min_samples=372` at 17.9k docs, i.e. "refuse to call anything a cluster under 744 documents". That, with `cluster_selection_method="leaf"`, is what leaves ~83% of run #4 unassigned — a tuning artefact, not an HDBSCAN property. Derive `min_cluster_size` from a target *cluster size* over an absolute range instead of a capped target count. Measure first: the shipped run dialog sets it per-run with no code change, so trying a value in the tens is a five-minute experiment. Blocks the comparison in `AGGLOMERATIVE_CLUSTERING.md` §5, which is otherwise rigged against a mis-tuned baseline.

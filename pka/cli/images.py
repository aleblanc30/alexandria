...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--ocr-lang", type=str, default=None, help="OCR language code(s), e.g. eng+fra"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Run an image text search instead of indexing (inferred text, plus CLIP when enabled)",
    )
    ...
    if args.search:
        ...
        for path, r in sorted(found, key=lambda pr: pr[1]["distance"]):
            log.info(
                ...
                r.get("image_type") or "?",
                path,
                r.get("filename") or r.get("document_id"),
                r["distance"],
            )
        return 0

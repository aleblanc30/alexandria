...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--fulltext", action="store_true", help="Run full-text extraction after metadata pass"
    )
    parser.add_argument(
        "--max-pages", type=int, default=None, help="Limit PDF pages per book during full-text pass"
    )
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument(
        "--force-reindex", action="store_true", help="Re-embed even books that already have chunks"
    )

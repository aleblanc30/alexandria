...
async def list_reading_lists(engine=Depends(get_engine)):
    with engine.connect() as con:
        ...
        for r in rows:
            n = con.execute(
                sa.select(sa.func.count()).select_from(reading_list_items)
...
async def list_items(list_id: int, engine=Depends(get_engine)):
    ...
    return [
        {"id": r[0], "position": r[1], "note": r[2],
         "doc_id": r[3], "title": r[4], "source": r[5],
         "url_or_path": r[6]}
        for r in rows
    ]
...
async def add_item(list_id: int, body: ItemAdd, engine=Depends(get_engine)):
    with engine.begin() as con:
        ...
        res = con.execute(reading_list_items.insert().values(
            list_id=list_id, document_id=body.document_id,
            position=max_pos + 1, note=body.note,
        ))
...
async def remove_item(list_id: int, item_id: int, engine=Depends(get_engine)):
    with engine.begin() as con:
        con.execute(
            ...
            .where((reading_list_items.c.id == item_id) &
...
async def delete_list(list_id: int, engine=Depends(get_engine)):
    with engine.begin() as con:
        con.execute(reading_list_items.delete()
                    .where(reading_list_items.c.list_id == list_id))
        con.execute(reading_lists.delete()
                    .where(reading_lists.c.list_id == list_id))

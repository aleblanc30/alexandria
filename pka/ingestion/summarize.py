...
_MAP_INTRO = (
    "You are indexing a document for a personal research library.\n"
...
_REDUCE_INTRO = (
    ...
    "ONE document. Merge them into a single summary of that document, dropping "
...
def _build_prompt(text: str, max_sentences: int, *, reduce: bool) -> str:
    intro = _REDUCE_INTRO if reduce else _MAP_INTRO
...
def _summarize_once(
    ...
    *,
    ...
    prompt = _build_prompt(text[:CHUNK_CHAR_LIMIT], max_sentences, reduce=reduce)
...
def _summarize_recursive(
    ...
    if len(text) <= CHUNK_CHAR_LIMIT or depth >= MAX_REDUCE_DEPTH:
        return _summarize_once(text, max_sentences, model, reduce=depth > 0)
    ...
    if len(chunks) <= 1:
        ...
        return _summarize_once(chunks[0] if chunks else text, max_sentences, model)

    partials: list[str] = []
    for index, chunk in enumerate(chunks, start=1):
        partial = _summarize_once(chunk, max_sentences, model, reduce=depth > 0)
        if partial:
    ...
    return _summarize_recursive(joined, max_sentences, model, depth + 1)
...
def summarize_text(
    ...
    *max_sentences* defaults to ``cfg.summary_max_sentences``. *model* overrides
    ...
    unchanged, costing no model call.
    """
    ...
    try:
        summary = _summarize_recursive(cleaned, limit, model, depth=0)

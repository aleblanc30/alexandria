...
def _seed_labeled_corpus() -> tuple[list[int], list[int], list[int]]:
    ...
    for i in range(4):
        doc_id = upsert_document(
            "zotero", f"P{i}", f"Positive {i}", "https://example.com/p", 1700000000 + i,
        )
        _set_embedding(doc_id, _pos_vec(1.0 + i * 0.01))
        pos_ids.append(doc_id)
    for i in range(4):
        doc_id = upsert_document(
            "firefox", f"N{i}", f"Negative {i}", "https://example.com/n", 1700000100 + i,
        )
        _set_embedding(doc_id, _neg_vec(-1.0 - i * 0.01))
        neg_ids.append(doc_id)
    for i in range(3):
        doc_id = upsert_document(
            "calibre", f"U{i}", f"Unlabeled {i}", "https://example.com/u", 1700000200 + i,
        )
...
class TestTagTrainingLifecycle:
    ...
    def test_from_source_tag(self):
        init_db()
        doc_id = upsert_document(
            "zotero", "ST1", "Tagged", "https://example.com/t", 1700000000,
        )
        for i, vec in enumerate([_neg_vec(), _neg_vec(), _neg_vec()]):
            nid = upsert_document(
                "firefox", f"STN{i}", f"Other{i}", f"https://example.com/n{i}", 1700000001 + i,
            )
        ...
        with eng.begin() as con:
            con.execute(
                source_tags.insert().values(
                    document_id=doc_id, tag_string="consensus", source="zotero",
                )
            )
    ...
    def test_pseudo_label_model_threshold(self, monkeypatch):
        ...
        sure_pos = upsert_document(
            "zotero", "SP", "Sure positive", "https://example.com/sp", 1700000300,
        )
        ...
        with get_engine().connect() as con:
            row = con.execute(
                sa.select(tag_training_labels.c.source, tag_training_labels.c.label)
                .where(
    ...
    def test_llm_pseudo_uses_user_negatives(self, monkeypatch):
        ...
        def fake_classify(tag, *, seed_samples, negative_samples, title, excerpt, model=None, negative_source="random"):
    ...
    def test_upsert_rejects_predicted_source(self):
        ...
        with pytest.raises(ValueError, match="Invalid label source"):
            with eng.begin() as con:
                lifecycle._upsert_labels(
                    con, session["session_id"], [(neg_ids[1], 1)], "predicted",
                )
...
class TestTagTrainingApi:
    def test_create_session_endpoint(self, client):
        pos_ids, neg_ids, _ = _seed_labeled_corpus()
        r = client.post("/tag-training/sessions", json={
            "tag": "api-tag",
            "labels": (
                [{"doc_id": did, "label": 1} for did in pos_ids[:3]]
    ...
    def test_from_source_tag_endpoint(self, client):
        doc_id = upsert_document(
            "zotero", "ST2", "Tagged", "https://example.com/t2", 1700000000,
        )
        _set_embedding(doc_id, _pos_vec())
        eng = get_engine()
        with eng.begin() as con:
            con.execute(
                source_tags.insert().values(
                    document_id=doc_id, tag_string="raft", source="zotero",
                )
            )
        r = client.post("/tag-training/sessions/from-source-tag", json={
            "source_tag": "raft",
    ...
    def test_queue_and_label_flow(self, client):
        ...
        labeled = client.post(f"/tag-training/sessions/{sid}/labels", json={
    ...
    def test_apply_learned_tag_to_new_document(self):
        ...
        new_id = upsert_document(
            "zotero", "NEW1", "New paper", "https://example.com/new", 1700000999,
        )
